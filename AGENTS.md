...
| Touching `crates/dodo-flow/` — the Flow Canvas engine, still being built and not yet in the sidebar | `crates/dodo-flow/src/lib.rs`'s doc comment, which carries the phase-by-phase state and the recorded numbers, then whichever of these the change touches: `budgets.rs` before changing anything that paints, `render/plan.rs` before adding a painter, `spatial/mod.rs` before anything that decides what is drawn, and `runtime/world.rs` before touching the graph — paint order, vertex accounting, culling and dirty propagation are contracts there, and **all four fail silently**: over the vertex ceiling the window renders solid black rather than slowly |
