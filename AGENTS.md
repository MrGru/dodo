...
| Touching `crates/dodo-flow/` — the Flow Canvas engine, still being built and not yet in the sidebar | `crates/dodo-flow/src/lib.rs`'s doc comment, which carries the phase-by-phase state and the recorded numbers, then whichever of these the change touches: `budgets.rs` before changing anything that paints, `render/plan.rs` before adding a painter, `spatial/mod.rs` before anything that decides what is drawn, `render/lod.rs` before anything that decides *how much* of it is drawn, `render/snapshot.rs` before anything a frame reads, `render/cache.rs` before anything cached across frames, and `runtime/world.rs` before touching the graph — paint order, vertex accounting, culling, simplification and dirty propagation are contracts there, and **all of them fail silently**: over the vertex ceiling the window renders solid black rather than slowly, and culling alone cannot bound a graph whose edges cross the document |
