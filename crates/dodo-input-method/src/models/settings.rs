//! `input-method.json`: dodo's input-language and Vietnamese-engine settings.
//!
//! This is ordinary application state: dodo is its only reader and writer, and
//! reconfigures its Event Tap or Keyboard Hook directly after every change.
//!
...
//!
//! - **A stable persisted format.** The engine's enums are free to gain
//!   variants, be reordered or be renamed; this file's strings are `"telex"`,
//!   `"vni"`, `"modern"` and `"traditional"` and keep their existing meaning.
//! - **A place to decide what is *not* a setting.** [`OutputMode`] is absent:
//!   both platform listeners use direct output, so there is no choice to save.
...
use crate::services::document::{SettingsError, parse_versioned, read_versioned, write_atomic};
...
///
/// The current number remains 8 so files written by earlier releases stay
/// readable. A future incompatible shape must bump it; additive defaulted fields
/// do not need to.
...
impl VietnameseSettings {
    ...
    ///
    /// [`OutputMode`] is supplied here rather than read from the file; each
    /// listener selects direct output at its own boundary.
...
pub struct SettingsDocument {
    ...
    ///
    /// Only dodo's macOS Event Tap reads this. It lives in this file rather than in
...
impl SettingsDocument {
    /// Carries arbitrary edited fields forward while stamping the current schema.
    pub fn next_from(mut next: SettingsDocument) -> SettingsDocument {
        next.version = SETTINGS_SCHEMA_VERSION;
        next
    }

    /// Reads the file, answering `None` when it does not exist yet.
    pub fn read(path: &std::path::Path) -> Result<Option<SettingsDocument>, SettingsError> {
    ...
    pub fn parse(bytes: &[u8]) -> Result<SettingsDocument, SettingsError> {
    ...
    pub fn write(&self, path: &std::path::Path) -> Result<(), SettingsError> {
...
mod tests {
    use super::{
        LanguageSwitch, SETTINGS_SCHEMA_VERSION, Scheme, SettingsDocument, Shortcut, ShortcutKey,
    ...
    fn the_browser_address_bar_fix_defaults_on_and_survives_a_file_that_never_heard_of_it() {
        ...
        // It is carried forward when the schema stamp is refreshed.
        assert!(!SettingsDocument::next_from(off).browser_address_bar_fix);
    }
    ...
    /// The strings in the file are a persisted format and stay stable.
    #[test]
    ...
    /// otherwise silently stop an existing installation recognising the shortcut.
    #[test]
    ...
    fn an_unknown_scheme_is_refused() {
        ...
        .unwrap_err();
        assert!(matches!(error, SettingsError::Io { .. }), "{error:?}");
    }
