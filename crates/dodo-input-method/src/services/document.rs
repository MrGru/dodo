//! The settings file's version rule and atomic write.
//!
//! Everything here is pure but for [`write_atomic`].
...
pub enum SettingsError {
    ...
    Io { detail: String },
    /// The file carries no numeric `version`.
    MissingVersion,
...
impl SettingsError {
    pub fn io(detail: impl Into<String>) -> SettingsError {
        SettingsError::Io {
            detail: detail.into(),
        }
    }
...
impl std::fmt::Display for SettingsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SettingsError::Io { detail } => write!(f, "{detail}"),
            SettingsError::MissingVersion => write!(f, "no schema version"),
...
///    is [`SettingsError::MissingVersion`].
...
///
/// Step 3 keeps a newer build's changed meanings from being guessed at.
pub fn parse_versioned<T: DeserializeOwned>(
    bytes: &[u8],
    supported: u32,
) -> Result<T, SettingsError> {
    let value: Value =
        serde_json::from_slice(bytes).map_err(|err| SettingsError::io(err.to_string()))?;
    ...
    serde_json::from_value(value).map_err(|err| SettingsError::io(err.to_string()))
}
    ...
    match std::fs::read(path) {
        Ok(bytes) => parse_versioned(&bytes, supported)
            ...
            .map_err(|error| match error {
                ...
                // it.
                SettingsError::Io { detail } => {
                    SettingsError::io(format!("{}: {detail}", path.display()))
                }
                other => other,
            }),
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(None),
        Err(err) => Err(SettingsError::io(format!("{}: {err}", path.display()))),
    }
}

/// Writes a document so a reader never sees half of it.
///
/// The temp file sits beside the target and is renamed over it atomically.
pub fn write_atomic<T: Serialize>(path: &Path, document: &T) -> Result<(), SettingsError> {
    if let Some(dir) = path.parent() {
        std::fs::create_dir_all(dir)
            .map_err(|err| SettingsError::io(format!("{}: {err}", dir.display())))?;
    }

    let json =
        serde_json::to_vec_pretty(document).map_err(|err| SettingsError::io(err.to_string()))?;

    let tmp = path.with_extension(format!("json.{}.tmp", std::process::id()));
    std::fs::write(&tmp, &json)
        .map_err(|err| SettingsError::io(format!("{}: {err}", tmp.display())))?;
    std::fs::rename(&tmp, path).map_err(|err| {
        // Leave nothing behind if the rename is the step that failed.
        let _ = std::fs::remove_file(&tmp);
        SettingsError::io(format!("{}: {err}", path.display()))
    })
}

#[cfg(test)]
mod tests {
    use super::{SettingsError, parse_versioned, read_versioned, write_atomic};
    ...
    fn a_file_with_no_version_is_refused_as_such() {
        ...
        assert_eq!(error, SettingsError::MissingVersion);
    }
    ...
    fn a_truncated_file_fails_as_malformed_json() {
        ...
        for cut in 1..whole.len() {
            ...
            assert!(
                matches!(error, SettingsError::Io { .. }),
        ...
        for body in [
            ...
            assert!(
                matches!(
                    ...
                    Err(SettingsError::Io { .. }) | Err(SettingsError::MissingVersion)
                ),
                "{:?} was accepted",
                String::from_utf8_lossy(body)
            );
    ...
    fn a_missing_file_is_not_an_error() {
        let dir =
            std::env::temp_dir().join(format!("dodo-input-settings-absent-{}", std::process::id()));
    ...
    fn a_written_document_reads_back_and_leaves_no_scratch_file() {
        let dir =
            std::env::temp_dir().join(format!("dodo-input-settings-write-{}", std::process::id()));
    ...
    /// The directory is created on demand on a fresh account.
    #[test]
    fn writing_creates_the_directory() {
        let dir = std::env::temp_dir()
            .join(format!("dodo-input-settings-mkdir-{}", std::process::id()))
            .join("nested");
        let _ = std::fs::remove_dir_all(&dir);
        let path = dir.join("probe.json");
