...
//! [`QueryTabs`](crate::state::tabs::QueryTabs). A tab's label is
...
use crate::views::database::{DatabaseView, EDITOR_HEIGHT, EDITOR_MIN};
use crate::views::result_grid;
...
impl DatabaseView {
    ...
    pub(super) fn render_edit_toolbar(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let (read_only, cell_error, row_error, duplicate_error, pending) = self
            ...
            .unwrap_or((
                Some(crate::models::identity::ReadOnlyReason::NoColumns),
                None,
                None,
                None,
                0,
