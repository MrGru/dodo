...
//!
//! **This is a *feature* crate.** It was `src/database/` until 2026-08-15,
//! when it moved out whole, following the shape `dodo-cleaner` set and
//! `dodo-docker` repeated: the binary links it and names four items —
//! [`DatabaseView`] and [`init`], plus [`models::uri`] and its
//! [`ParsedUri`](models::uri::ParsedUri), which `quick_nav` reads to route a
//! pasted connection string — while `main.rs` aliases the crate back to
//! `crate::database`, so no call site on either side of the seam changed. Its
//! outbound edges were only the three kernel crates (`dodo-app-icon`,
//! `dodo-i18n`, `dodo-paths`), which is what made it extractable at all.
//! `Cargo.toml` says why each remaining dependency is there. Two seams exist
//! only because of the move: [`paths`] supplies the one impure input
//! `dodo_paths`' pure rules take, and the `app_icon` / `i18n` aliases below
//! keep `crate::app_icon::AppIcon` and `crate::i18n::t` spelled as they were.
//!
//! **The module declarations below say `pub(crate)` where they can, and that
//! is not a narrowing.** Inside a binary crate `pub` already meant "reachable
//! from dodo and nowhere else"; spelling it `pub(crate)` here is what keeps
//! that true now that there is an outside. [`models`] is the one that stays
//! `pub`, because `quick_nav` reads [`models::uri`] across the seam — the
//! module's own contents were already `pub` inside the binary and nothing
//! about their reachability changed.
//!
//! **35 of the 52 files here name no UI framework**, and that is a contract
//! rather than an accident: `models`, `services` and `state` are tested with
//! no `App` and no frame.
//!
...
//! particular it does **not** name `dodo_docker`: the design report proposed
...
//! names another tool.** `grep -rn '^use crate::' crates/dodo-database/src/ |
//! grep -vE 'crate::(i18n|app_icon|paths)'` returns nothing — the anchor keeps
//! the check from matching its own description. Being a crate of its own is
//! now the second half of that guarantee: an edge onto another tool would have
//! to be a line in `Cargo.toml` first. Other modules are
...
// still reads `crate::app_icon::AppIcon` and `crate::i18n::{database, t}`,
