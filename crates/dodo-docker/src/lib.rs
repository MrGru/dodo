...
//!
//! **This is a *feature* crate.** It was `src/docker/` until 2026-08-15, when
//! it moved out whole, following the shape `dodo-cleaner` set: the binary
//! links it and names exactly three items — [`DockerPage`], [`DockerView`] and
//! [`init`] — while `main.rs` aliases the crate back to `crate::docker`, so no
//! call site on either side of the seam changed. Its outbound edges were only
//! the three kernel crates (`dodo-app-icon`, `dodo-i18n`, `dodo-paths`), which
//! is what made it extractable at all, and `layout.rs` was its only inbound
//! one. `Cargo.toml` says why each remaining dependency is there. Two seams
//! exist only because of the move: [`paths`] supplies the one impure input
...
//! narrowing.** Inside a binary crate `pub` already meant "reachable from dodo
//! and nowhere else"; spelling it `pub(crate)` here is what keeps that true
//! now that there is an outside.
//!
//! **27 of the 43 files here name no UI framework**, and that is a contract
...
// still reads `crate::app_icon::AppIcon` and `crate::i18n::{docker, t}`,
...
pub(crate) const KEY_CONTEXT: &str = "DockerList";
