...
use crate::state::filters::ContainerFilters;
use crate::state::grouping::{ContainerGroup, GroupKey, group_containers};
...
impl ContainersState {
    ...
    pub fn merge_rows(&mut self, mut incoming: Vec<Container>) -> bool {
        crate::state::diff::carry_cpu(&self.rows, &mut incoming);
        sort_for_display(&mut incoming);
        let rows_differ = crate::state::diff::rows_changed(&self.rows, &incoming);
...
mod tests {
    ...
    fn filters_and_search_narrow_together() {
        use crate::state::grouping::GroupKey;
