...
//! ([`views::detail`](crate::views::detail)).
...
use crate::components::states::{empty_state, error_state};
...
use crate::services::{DockerEngine, default_engine};
use crate::state::containers::LoadStatus;
use crate::state::focus::{FocusMove, next_focus};
use crate::state::resource::ResourceState;
use crate::views::detail::{self, DetailRequest};
...
impl VolumesView {
    ...
    ///
    /// [`DockerView`]: crate::views::DockerView
    ...
    /// Re-lists the volumes and their usage every [`POLL_INTERVAL`] and merges the
        ...
        self.poll_task = Some(cx.spawn(async move |this, cx| {
            loop {
                ...
                let result = cx
                    .background_executor()
                    .spawn(async move {
                        let volumes = fetch.list_volumes()?;
                        let usage = fetch.container_usage().unwrap_or_default();
                        Ok::<_, crate::services::DockerError>((sorted(volumes), usage))
                    })
                    .await;
                if this
    ...
    fn render_row(&self, row: Volume, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        h_flex()
            .id(SharedString::from(format!("vrow-{}", row.name)))
            .w_full()
            .min_w(TABLE_MIN_W)
            .flex_shrink_0()
            .items_center()
            ...
            // Right-click: Delete plus the disabled Inspect placeholder.
...
impl Focusable for VolumesView {
