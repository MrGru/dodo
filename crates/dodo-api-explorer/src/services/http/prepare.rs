...
use crate::models::key_value::effective_pairs;
use crate::models::request::RequestDraft;
use crate::services::http::{auth, headers, request_body};
use crate::services::{PreparedRequest, TransportError};
...
mod tests {
    ...
    fn an_api_key_in_the_query_lands_after_the_param_rows() {
        ...
        d.auth = AuthDraft {
            ...
            key_location: crate::models::auth::ApiKeyLocation::Query,
            ..AuthDraft::default()
        };
        let prepared = prepare(&d).expect("should prepare");
        assert_eq!(
