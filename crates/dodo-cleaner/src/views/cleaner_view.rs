...
use crate::core::category::{CleanerCategory, CleanerSection};
use crate::core::errors::{CleanupError, ScanError};
use crate::core::ignore::{IgnoredItemsDocument, path_signature};
use crate::core::item::{CleanableItem, CleanableItemId};
#[cfg(target_os = "linux")]
use crate::core::item::{InstalledAppAction, ItemMetadata};
#[cfg(target_os = "macos")]
use crate::core::permissions::{MacPermission, PermissionService, PermissionState};
use crate::core::progress::LatestProgress;
#[cfg(not(any(target_os = "macos", target_os = "windows", target_os = "linux")))]
use crate::core::report::CleanupReport;
use crate::core::report::{CategoryScanResult, PartialScanReason, ScanCompleteness};
use crate::core::risk::SelectionPolicy;
use crate::core::scan_context::ScanContext;
use crate::core::scan_state::{ScanIndicator, ScanState};
use crate::core::scanner::CleanerScanner;
use crate::docker_cache;
use crate::i18n::{Str, cleaner, shell, t};
#[cfg(target_os = "macos")]
use crate::macos::applications::review as uninstall_review;
#[cfg(target_os = "macos")]
use crate::macos::{cleanup, permissions, platform};
use crate::services::ignore_store::{
    DiskOrphanIgnoreStore, OrphanIgnoreStore, OrphanIgnoreStoreError,
};
use crate::state::{CategoryState, CleanerState, default_scanners};
use crate::views::results_sync::{ResultsSync, ResultsSyncKey, ResultsSyncPlan};
use crate::views::results_table::{ResultsTableDelegate, category_icon};
#[cfg(target_os = "macos")]
use crate::views::uninstall_review_dialog;
...
pub struct CleanerView {
    ...
    /// [`crate::state::CleanerState`] for why those stay three
    ...
    /// `crate::services::ignore_store`. Not `#[cfg(target_os = "macos")]`
...
impl CleanerView {
    ...
    pub(super) fn reveal_in_finder(
        ...
        let result = crate::windows::platform::reveal_in_explorer(path.as_path());
        #[cfg(target_os = "linux")]
        let result = crate::linux::platform::reveal_in_file_manager(path.as_path());
        #[cfg(not(any(target_os = "macos", target_os = "windows", target_os = "linux")))]
        ...
        let task = cx.spawn(async move |this, cx| {
            let report = cx
                ...
                .spawn(async move {
                    ...
                    {
                        let _ = items;
                        crate::core::report::CleanupReport {
                            successes: Vec::new(),
                            failures: Vec::new(),
            ...
            let report = cx
                ...
                .spawn(async move {
                    if is_docker {
                        ...
                        #[cfg(target_os = "windows")]
                        {
                            crate::windows::cleanup::cleanup_items(&items)
                        }
                        #[cfg(target_os = "linux")]
                        {
                            crate::linux::cleanup::cleanup_items(&items)
                        }
                        #[cfg(not(any(
                            target_os = "macos",
                            target_os = "windows",
    ...
    pub(super) fn begin_uninstall_review(
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            let _ = item;
            if let Err(error) = crate::windows::platform::open_installed_apps_settings() {
                window.open_alert_dialog(cx, move |alert, _, cx| {
                    alert
                        .title(t(cleaner::Text::StatusFailed, cx))
                        .description(error.clone())
    ...
    fn pending_result(category: CleanerCategory) -> CategoryScanResult {
        CategoryScanResult {
            ...
            warnings: vec![crate::core::report::ScanWarning {
    ...
    fn render_cleanup_report_block(
        &self,
        report: &crate::core::report::CleanupReport,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        v_flex()
            .gap_1()
...
mod tests {
    ...
    use crate::core::progress::{LatestProgress, ProgressSink, ScanPhase, ScanProgress};
