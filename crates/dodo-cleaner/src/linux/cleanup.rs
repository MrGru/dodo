...
use crate::core::item::{CleanableItem, InstalledAppAction, ItemMetadata};
...
use crate::linux::platform::move_to_trash;
...
pub fn cleanup_items(items: &[CleanableItem]) -> CleanupReport {
    ...
    for path in dedupe_nested_paths(host, by_path.keys().cloned().collect()) {
        ...
        if let Some(application_id) = user_flatpak_action(item) {
            match crate::linux::scanners::installed_apps::uninstall_user_flatpak(application_id) {
...
fn user_flatpak_action(item: &CleanableItem) -> Option<&str> {
    ...
    if metadata.scope != crate::core::item::InstalledAppScope::User {
        return None;
    }
