...
//! dodo's own `api_explorer::services::consent_store` and
//! `quick_nav::services::config_store`: a trait, a disk
...
pub fn parse_document(bytes: &[u8]) -> Result<IgnoredItemsDocument, OrphanIgnoreStoreError> {
    ...
    if version > u64::from(crate::core::ignore::SCHEMA_VERSION) {
        return Err(OrphanIgnoreStoreError::UnsupportedVersion {
            found: version,
            understood: crate::core::ignore::SCHEMA_VERSION,
        });
    }
...
mod tests {
    ...
    use crate::core::ignore::{IgnoredItemsDocument, SCHEMA_VERSION};
