...
impl Drop for MacWindow {
    fn drop(&mut self) {
        ...
        // The GPUIView is installed as a subview of the window's content view and
        // holds a strong `Arc<Mutex<MacWindowState>>` in its `WINDOW_STATE_IVAR`.
        // AppKit keeps a content subview (with its first-responder, tracking-area,
        // layer-backing and input-context associations) retained while it stays
        // installed in a window, so closing the window is not enough to deallocate
        // it: `dealloc` never runs, its ivar `Arc` is never released, and the whole
        // `MacWindowState` — including the `MetalRenderer`'s `CAMetalLayer` and its
        // IOSurfaces — leaks on every window close. Detaching the view from its
        // superview lets AppKit tear those associations down so the view
        // deallocates and releases its state.
        ...
        // Deferred, not run inline: removing the view resigns first responder and
        // fires AppKit callbacks that re-enter `get_window_state().lock()`, which
        // would deadlock against the `this` guard still held here.
