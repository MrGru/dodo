...
fn pages(layout: &WeakEntity<Layout>, highlight: Option<Setting>, cx: &App) -> Vec<SettingPage> {
    ...
    vec![
        ...
        // The input method page is **last**, and macOS-only, and both are
        // deliberate. Last, because `Setting::page_ix` numbers the pages this
        // function returns and appending is the one change that moves no existing
        // index. macOS-only, because the thing it installs is an InputMethodKit
        // bundle: an install button that could not work is worse than no page.
        //
        // Nothing on it is in [`Setting::all`], so the search box does not offer
        // it — which is what keeps the indices above true on every platform
        // rather than only on this one. The library's own keyword search still
        // finds the rows.
        #[cfg(target_os = "macos")]
        input_method_page(cx),
    ]
}

/// The Input method page: install the bundle, and tell it how to type.
///
/// Every control here writes `input-method.json` and posts one notification;
/// `crate::input_method` is where that happens and why the order is what it is.
/// Nothing on this page starts, stops or talks to the input-method process —
/// dodo cannot, and the module docs say so at length.
///
/// The **first row is a status row with the install button in it**, rather than an
/// "Install" row and a separate "Status" row. There is only one thing to say and
/// one thing to do, and splitting them would mean two rows that are each half an
/// answer.
#[cfg(target_os = "macos")]
fn input_method_page(cx: &App) -> SettingPage {
    ...
    let mut group = SettingGroup::new()
        .title(title.clone())
        // The one thing worth saying about the whole page, said once at the top:
        // what an input method *is* here, and that it outlives dodo. Every row
        // below is then about one decision rather than repeating the premise.
        .description(t(Str::InputMethodDescription, cx))
        .item(
            SettingItem::new(
                t(Str::InputMethodStatus, cx),
                SettingField::render(|_, _, cx| install_button(cx)),
            )
            .description(t(install_status(cx), cx))
            .keywords([title.clone()]),
        )
        .item(
            SettingItem::new(t(Str::InputMethodScheme, cx), scheme_field(cx))
                .description(t(Str::InputMethodSchemeDescription, cx))
                .keywords([title.clone()]),
        )
        .item(
            SettingItem::new(
                t(Str::InputMethodTonePlacement, cx),
                tone_placement_field(cx),
            )
            .description(t(Str::InputMethodTonePlacementDescription, cx))
            .keywords([title.clone()]),
        )
        .item(
            SettingItem::new(
                t(Str::InputMethodSpellCheck, cx),
                SettingField::switch(
                    |cx: &App| InputMethod::settings(cx).spell_check,
                    |value: bool, cx: &mut App| InputMethod::set_spell_check(value, cx),
                )
                .default_value(true),
            )
            .description(t(Str::InputMethodSpellCheckDescription, cx))
            .keywords([title.clone()]),
        )
        .item(
            SettingItem::new(
                t(Str::InputMethodBracketShortcuts, cx),
                SettingField::switch(
                    |cx: &App| InputMethod::settings(cx).bracket_shortcuts,
                    |value: bool, cx: &mut App| InputMethod::set_bracket_shortcuts(value, cx),
                )
                .default_value(true),
            )
            .description(t(Str::InputMethodBracketShortcutsDescription, cx))
            .keywords([title.clone()]),
        );

    // Only when there is something wrong with the file — same treatment as
    // quick navigation's storage row and the session's. A refused settings file
    // here usually means the installed bundle and this dodo are different
    // versions, which is why the message says to reinstall rather than "update".
    if let Some(problem) = InputMethod::store_error(cx) {
        group = group.item(
            SettingItem::new(
                t(Str::InputMethodStorageProblem, cx),
                SettingField::render(|_, _, _| div()),
            )
            .description(t(problem, cx))
            .keywords([title.clone()]),
    ...
    SettingPage::new(title)
        ...
        // source, so it reads as "input" here rather than as "network".
        .icon(AppIcon::Globe)
        .resettable(false)
        .group(group)
}

/// What the status row says, in priority order.
///
/// The most recent *action* wins over the standing state: someone who has just
/// pressed the button wants to know what it did, and the install outcome is the
/// only place a refused `TISSelectInputSource` can be reported.
#[cfg(target_os = "macos")]
fn install_status(cx: &App) -> Str {
    ...
    match InputMethod::install_state(cx) {
        ...
        Install::Idle if !InputMethod::is_installed(cx) => Str::InputMethodNotInstalled,
        Install::Idle if InputMethod::settings_applied(cx) == Some(false) => {
            Str::InputMethodSettingsPending
        }
        Install::Idle => match InputMethod::status(cx) {
            Some(status) if status.describes_a_live_process() => {
                Str::InputMethodRunning(status.bundle_version)
            }
            // Installed, and either it has never run or the process that wrote
            // the status file has since exited — which is the ordinary state, not
            // a fault: macOS stops the agent when nothing is typing at it.
            _ => Str::InputMethodInstalledIdle,
        },
    }
}
...
fn install_button(cx: &App) -> Button {
    ...
    Button::new("install-input-method")
        ...
        // Input Sources calls and the `pkill` all run on the background executor
        // — so this closure only starts it. See `InputMethod::install`.
        .on_click(|_, _, cx| InputMethod::install(cx))
}

#[cfg(target_os = "macos")]
fn scheme_field(cx: &App) -> SettingField<SharedString> {
    use crate::input_method::InputMethod;
    use dodo_ime_ipc::settings::Scheme;

    let options = Scheme::ALL
        .map(|scheme| {
            let label = match scheme {
                Scheme::Telex => Str::InputMethodTelex,
                Scheme::Vni => Str::InputMethodVni,
            };
            (scheme.code().into(), t(label, cx))
        })
        .to_vec();

    SettingField::dropdown(
        options,
        |cx: &App| InputMethod::settings(cx).scheme.code().into(),
        |value: SharedString, cx: &mut App| {
            // An unknown code cannot come from the dropdown, and falling back to
            // the current value means a stray one changes nothing rather than
            // silently resetting the user's scheme.
            if let Some(scheme) = Scheme::from_code(&value) {
                InputMethod::set_scheme(scheme, cx);
            }
        },
    )
    .default_value(Scheme::default().code())
}

#[cfg(target_os = "macos")]
fn tone_placement_field(cx: &App) -> SettingField<SharedString> {
    use crate::input_method::InputMethod;
    use dodo_ime_ipc::settings::Tone;

    let options = Tone::ALL
        .map(|tone| {
            let label = match tone {
                Tone::Modern => Str::InputMethodToneModern,
                Tone::Traditional => Str::InputMethodToneTraditional,
            };
            (tone.code().into(), t(label, cx))
        })
        .to_vec();

    SettingField::dropdown(
        options,
        |cx: &App| InputMethod::settings(cx).tone_placement.code().into(),
        |value: SharedString, cx: &mut App| {
            if let Some(tone) = Tone::from_code(&value) {
                InputMethod::set_tone_placement(tone, cx);
            }
        },
    )
    .default_value(Tone::default().code())
}
