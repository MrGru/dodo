...
//! one file per language inside it, and a global holding the active choice.
//!
//! Each area under this directory owns its own strings and its own
...
macro_rules! areas {
    ($($module:ident => $variant:ident),+ $(,)?) => {
        ...
        impl Str {
            ...
            ///
            /// `pub(crate)` rather than private because a [`Str`] can be *held*
            /// rather than rendered on the spot — a `ConsoleEntry` keeps dodo's
            /// own lines unrendered so they re-translate — and those holders
            /// have to be testable without a `Window`. Views go through [`t`].
